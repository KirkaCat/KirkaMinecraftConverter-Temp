...
export default (function () {
    ...
    for (const fileName of dataFileNames) {
        const dataName = fileName.replace(/^(.+)\.[^\.]+/, (_, name) => {
            return name.replaceAll(/(^|\-)(\w)/g, (_, __, char) => char.toUpperCase()); // Convert to PascalCase
        });
        const fileData = fs.readFileSync(`${NBT_DATA_PATH}/${fileName}`);
        const nbtData = nbt.parseUncompressed(pako.ungzip(fileData));
