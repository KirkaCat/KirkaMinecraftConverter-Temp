...
const radiansToMinecraft = (radians) => {
    const degrees = radians * (180 / Math.PI);
    return (((-degrees - 180) + 180) % 360) - 180;
};
function minecraftToRadians(degrees) {
    return ((((-degrees - 180) * (Math.PI / 180)) % (2 * Math.PI)) + (2 * Math.PI)) % (2 * Math.PI);
}
function _toMinecraft(mapData, options) {
    options = options ?? {};
    ...
    const translationPalette = new Set([BlockTranslation.get(0)]); // Initialize the block palette with the air translation.
    const blockData = [];
    ...
    for (const chunkCoordStr of Object.keys(mapData.chunks)) {
        ...
        for (const block of decompressedChunkData) {
            ...
            if (translation === undefined)
                throw new Error(`No block translation found for Kirka block ID: ${blockId}`);
            ...
            blockData.push({
                translation: BlockTranslation.get(blockId),
                x: gx,
                y: gy,
                z: gz,
            });
    ...
    const team1SpawnData = [];
    const team2SpawnData = [];
    const regionData = [];
    ...
    const [paletteMap, schematicBlockData] = [Object.fromEntries([...translationPalette.values()].map((translation, index) => [translation.minecraft.blockStateId, index])), []];
    ...
    const entityNbtData = { type: 'list', value: { type: 'compound', value: [] } };
    if (team1SpawnData.length > 0) {
        for (const i in team1SpawnData) {
            ...
            const entityNbt = JSON.parse(JSON.stringify(KirkaMarkerBlueSpawn)).value.components.value["minecraft:entity_data"];
    ...
    if (team2SpawnData.length > 0) {
        for (const i in team2SpawnData) {
            ...
            const mcRotation = radiansToMinecraft(spawn.rotation);
            const entityNbt = JSON.parse(JSON.stringify(KirkaMarkerRedSpawn)).value.components.value["minecraft:entity_data"];
            entityNbt.value.Rotation.value.value[0] = mcRotation;
            entityNbt.value.PortalCooldown.value = entityNbt.value.PortalCooldown.value - Number(i);
            entityNbtData.value.value.push({
                // @ts-ignore
    ...
    if (regionData.length > 0) {
        for (const i in regionData) {
            ...
            const entityNbt = JSON.parse(JSON.stringify(KirkaMarkerRegionDebug)).value.components.value["minecraft:entity_data"];
            entityNbt.value.PortalCooldown.value = entityNbt.value.PortalCooldown.value - Number(i);
            entityNbtData.value.value.push({
                // @ts-ignore
                Data: { type: 'compound', value: entityNbt.value },
    ...
    if (checkpointData.length > 0) {
        for (const i in checkpointData) {
            ...
            const entityNbt = JSON.parse(JSON.stringify(KirkaMarkerCheckpointDebug)).value.components.value["minecraft:entity_data"];
            entityNbt.value.PortalCooldown.value = entityNbt.value.PortalCooldown.value - Number(i);
            entityNbtData.value.value.push({
                // @ts-ignore
                Data: { type: 'compound', value: entityNbt.value },
