...
export class BlockTranslation {
    ...
    canTranslate(id, blockStates, strict = false) {
        if (this.kirka === id)
            return true;
        if (typeof id === 'string')
            return this.minecraft.is(id, blockStates, strict);
        return false;
    }
    canResolve(id, blockStates, strict = false) {
        if (this.resolvable === undefined)
            return false;
        for (const translation of this.resolvable)
            if (translation.is(id, blockStates, strict))
                return true;
        return false;
    }
}
class MinecraftBlockTranslation {
    ...
    get blockStateId() {
        ...
        for (const property of Object.keys(this.blockStates)) {
            const state = this.blockStates[property];
            propertyStates.push(`${property}=${state}`);
        }
        return `${this.id}[${propertyStates.sort().join(',')}]`;
    }
    is(id, blockStates, strict = false) {
        if (this.id !== id)
            return false;
        if (this.blockStates === undefined || blockStates === undefined)
            return true;
        if (strict &&
        ...
        for (const property of Object.keys(blockStates)) {
            const state = blockStates[property];
            if (!(property in this.blockStates) || this.blockStates[property] !== state)
                return false; // Property does not exist, or state does not match.
        }
        return true;
    }
}
const _referenceMap = new Map();
export const _BlockTranslationReference = {
...
for (const [strId, translation] of Object.entries(_BlockTranslationReference)) {
    const kirkaBlockId = Number(strId);
    if (translation === null)
        continue;
    // Primary translation
    const { id: primaryId, ...blockStates } = translation.primary;
    const primaryTranslation = new MinecraftBlockTranslation(primaryId, blockStates);
    ...
    const resolvableTranslations = translation.resolvable?.map(resolvable => {
        ...
        return new MinecraftBlockTranslation(resolvableId, resolvableStates);
    });
    ...
    const translationSet = _referenceMap.get(primaryId) ?? new Set();
